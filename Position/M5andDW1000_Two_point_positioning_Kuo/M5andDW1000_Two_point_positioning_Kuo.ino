...
void UWB_display(){
  ...
  if (UWB_T_NUMBER > 0 && UWB_T_NUMBER < 5){
    ...
    while (c > 0){
      ...
      //Serial.print(DATA.substring(2 + c * 11, 3 + c * 11));
      if (DATA.substring(2 + c * 11, 3 + c * 11) == "0"){
        distan0 = DATA.substring(4 + c * 11, 8 + c * 11).toFloat();
        }
      else{
        distan1 = DATA.substring(4 + c * 11, 8 + c * 11).toFloat();
        }
    double a = distan1;     //The length of the opposite side of angle A
    double b = distan0;     //The length of the opposite side of angle B
    double c = 2.0;         //The length of the opposite side of angle C
    double xc1 = 0;         //x coordinate of angle A
    double yc1 = 0;         //y coordinate of angle A
    double xc2 = 2.0;       //x coordinate of angle B
    double yc2 = 0;         //y coordinate of angle B

    double angleAB= acos((sq(b)+sq(c)-sq(a))/(2*b*c));
    double x =  xc1 + b*cos(angleAB);
    double y =  yc1 + b*sin(angleAB);
   
    M5.Lcd.drawString(String(x), 100, 50 + 3 * 40, 4 );
    M5.Lcd.drawString(String(y), 210, 50 + 3 * 40, 4 );
